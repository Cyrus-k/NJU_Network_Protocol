...
void* listen_to_neighbor(void* arg) {
	...
	while(1){
		if(recvpkt(&pkt,nt[no].conn) > 0){
			...
			if(pkt.header.type == 3){
				if(pkt.header.src_nodeID == nt[no].nodeID){
					close(nt[no].conn);
					printf("good\n");
				}
			}
...
void waitSIP() {
	...
	pkt.header.length = 0;
	for(int i = 0;i<neighbor_num;i++){
		pkt.header.dest_nodeID = nt[i].nodeID;
		if(sendpkt(&pkt,nt[i].conn)){
			printf("succeed sending disconnect to neighbour\n");
		}
	}
	printf("sip disconnect\n");
}
...
int main() {
	...
	int i;
	for(i=0;i<nbrNum;i++) {
		printf("Overlay network: neighbor %d:%d , distance:%d\n",i+1,nt[i].nodeID,nt[i].dis);
	}
